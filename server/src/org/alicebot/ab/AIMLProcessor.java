...
*/
    ...
    public static String evalTemplate(String template, ParseState ps) {
        ...
        try {
        	// remove randoms - replace with multiple responses 
        	if (template.startsWith("<random>")) {
        		final int RANDOM_LENGTH = 8;
        		final int END_RANDOM_LENGTH = 9;
        		
        		template = template.replace("\n",  "").replace("<li>", "").replace("</li>", "|");
        		template = template.substring(RANDOM_LENGTH, template.length() - END_RANDOM_LENGTH - 1);
        	}
